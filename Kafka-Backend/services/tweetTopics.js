...
let replyATweet = async (message, callback) => {
  ...
  tweet.update(
    { _id: tweetId },
    { $push: { replies: { username, userFullName, profilePic, replyText } } },
    (err, result) => {
      if (err) {
        console.log("unable to insert into database", err);
        callback(err, "Database Error");
...
let likeATweet = async function(message, callback) {
  ...
  tweet.update(
    { _id: tweetId },
    { $push: { likes: { username, userFullName, profilePic } } },
    (err, result) => {
      if (err) {
        console.log("unable to insert into database", err);
        callback(err, "Database Error");
    ...
    if (err) {
      ...
      if (result) {
        ...
        if (followingActive && followingActive.length > 0) {
          await tweet
            .find({ username: { $in: followingActive } })
            .lean()
            .exec(async (err, result) => {
              if (err) {
                ...
                callback(null, {
                  status: 401,
                  message: "tweets list cannot be returned!!"
                });
              }
            });
        } else {
  ...
  tweet.findOneAndUpdate(
    ...
    (err, result) => {
      if (err) {
        ...
        callback(null, {
          status: 401,
          message: "tweet details cannot be returned!"
        });
      }
    }
  );
  ...
  tweet.update(
    ...
    (err, result) => {
      if (err) {
        ...
        callback(null, {
          status: 200,
          message: "tweet  cannot be bookmarked!!"
        });
      }
    }
  );
...
let retweetWithoutComment = async (message, callback) => {
  ...
  tweet.update(
    { _id: tweetId },
    { $push: { retweets: { username, userFullName, profilePic } } },
    (err, result) => {
      if (err) {
        console.log("unable to delete into database", err);
        callback(err, "Database Error");
