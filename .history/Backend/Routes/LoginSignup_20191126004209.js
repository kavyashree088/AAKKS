...
router.post('/deleteAccount', function (req, res) {
  ...
  kafka.make_request('loginSignuptopic', { "path": "deleteAccount", "username": req.query.username }, function (err, result) {
    console.log(result)
    if (err) {
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
...
router.post('/deactivateAccount', function (req, res) {
  ...
  kafka.make_request('loginSignuptopic', { "path": "deactivateAccount", "username": req.query.username }, function (err, result) {
    console.log(result)
    if (err) {
      res.status(500).json({ responseMessage: 'Database not responding' });
    }
      ...
      req.session.user = result.username;
      res.status(200).json({ validUser: true, responseMessage: 'Deactivation Successfully' });
